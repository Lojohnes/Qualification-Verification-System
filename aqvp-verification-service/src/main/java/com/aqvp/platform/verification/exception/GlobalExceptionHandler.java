...
/**
 * Global REST exception handler for the verification module.
 */
@Slf4j
...
public class GlobalExceptionHandler {
    ...
    @ExceptionHandler({InvalidQrException.class, BusinessException.class})
    ...
    public ResponseEntity<ErrorResponse> handleUpstream(UpstreamServiceException ex, HttpServletRequest request) {
        ...
        return ResponseEntity.status(HttpStatus.SERVICE_UNAVAILABLE)
            .body(buildErrorResponse(HttpStatus.SERVICE_UNAVAILABLE, ex.getMessage(), request, null));
    }
    ...
    public ResponseEntity<ErrorResponse> handleGeneric(Exception ex, HttpServletRequest request) {
        log.error("Unexpected verification error", ex);
